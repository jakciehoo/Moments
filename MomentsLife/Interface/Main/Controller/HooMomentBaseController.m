...
@interface HooMomentBaseController ()<HooCreateQuoteViewControllerDelegate,HooProductPickViewDelegate>
...
@property (nonatomic, weak)HooProductPickView *productPickView;
...
- (void)setupView:(HooMoment *)moment
{
    ...
    if (!self.fromMine) {
        NSString *imagePath = [[NSBundle mainBundle] pathForResource:moment.photo.image_filename ofType:nil];
...
- (void)tapToHideButtons
{
    ...
    [self.centerButton showOrHideSubviews];
    //关闭产品选择器
    if (self.productPickView) {
        [self hideProductPickView];
    }
...
- (IBAction)pinButtonClicked:(UIButton *)sender {
    UIImage *image = [UIImage imageDrawInView:self.imageView Inrect:self.imageView.frame];
    //保存快照的缩略图片到沙盒
...
- (void)printMoment
{
    HooProductPickView *pickView = [[HooProductPickView alloc] init];
    pickView.delegate = self;

    pickView.backgroundColor = [self.imageView.image mostColorFromImage];
    [pickView showInView:self.view];
    [pickView autoPinEdgesToSuperviewEdgesWithInsets:UIEdgeInsetsZero excludingEdge:ALEdgeTop];
    [pickView autoSetDimension:ALDimensionHeight toSize:200];
    _productPickView = pickView;
}

- (void)pickerDidSelectProduct:(HooProduct *)product
{
    UIImage *printImage = [UIImage imageDrawInView:self.imageView Inrect:self.imageView.frame];
    HooEditViewController *editViewCtrl = [[HooEditViewController alloc] init];
    [editViewCtrl setValue:printImage forKey:@"_printImage"];
    [editViewCtrl setValue:product forKey:@"_product"];
    HooStopAutoRotationNaviController *navi = [[HooStopAutoRotationNaviController alloc] initWithRootViewController:editViewCtrl];
    [self presentViewController:navi animated:YES completion:nil];

}
- (void)hideProductPickView
{
    [self.productPickView cancelPicker];
    self.productPickView.delegate =  nil;
    self.productPickView = nil;
}
...
- (void)shareMoment:(id)sender
{
    ...
    [HooShareTool showshareAlertControllerIn:sender WithImage:image andFileName:imageName];
...
#pragma mark - HooCreateQuoteViewControllerDelegate
