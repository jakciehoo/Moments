...
@interface HooMyPostersController ()<HooProductPickViewDelegate>
...
- (void)showMenu:(HooMoment *)moment
{
    ...
    UIAlertAction *shareAction = [UIAlertAction actionWithTitle:@"分享" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        [HooShareTool showshareAlertControllerIn:self.selectedCell WithImage:image andFileName:image_thumbName];
    }];
...
- (void)deleteMoment:(HooMoment *)moment
{
    ...
    if (!flag) {
        ...
        [self.myPosters removeObject:moment];
        [self.tableView reloadData];
    }
}

- (void)printMoment:(HooMoment *)moment
{
    NSString *image_thumbName = moment.photo.image_thumb_filename;
    self.printImage = [UIImage imageForPhotoName:image_thumbName];
    
    HooProductPickView *pickView = [[HooProductPickView alloc] init];
    pickView.delegate = self;
    
    [pickView showInView:HooKeyWindow];
...
{
    if (self.printImage == nil) return;
    HooEditViewController *editViewCtrl = [[HooEditViewController alloc] init];
    [editViewCtrl setValue:self.printImage forKey:@"_printImage"];
