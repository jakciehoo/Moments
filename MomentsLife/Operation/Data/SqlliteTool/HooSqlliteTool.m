...
//    flag = [_db executeUpdate:@"CREATE TABLE IF NOT EXISTS t_address (id integer PRIMARY KEY,name text,phone_number text, zipCode text,country_address text,detail_address text,is_default integer)"];
//    if (flag) {
//        HooLog(@"创建表格t_address成功");
//    }else{
//        HooLog(@"创建表格t_address失败");
//    }
}
/*----------------------------查数据----------------------------------------*/
+ (NSArray *)getMyMomentsBefore:(NSString *)date LimitCount:(NSInteger)count
{
    NSString *sql =[NSString stringWithFormat:@"SELECT * FROM t_myMoment WHERE modified_date < %@ ORDER BY modified_date DESC limit %ld",date ,(long)count] ;
...
+ (NSArray *)momentsBeforeDate:(NSString *)date andIsStared:(BOOL)isStared containText:(NSString *)text limitCount:(NSInteger)count
{
    ...
    if (count) {
        NSString *countStr = [NSString stringWithFormat:@" order by modified_date desc limit %ld",(long)count];
...
+ (NSArray *)momentResultsWithSql:(NSString *)sql
{
    ...
    while ([set next]) {
        ...
        moment.fontSize = [set doubleForColumn:@"fontSize"];
...
{
    NSString *sql = [NSString stringWithFormat:@"DELETE FROM t_moment WHERE id = %ld",(long)ID];
    BOOL flag = [_db executeUpdate:sql];
    if (flag) {
        HooLog(@"删除数据成功");
    }else{
...
//+ (NSArray *)addresses
//{
//    NSString *sql =[NSString stringWithFormat:@"SELECT * FROM t_address"] ;
//    FMResultSet *set = [_db executeQuery:sql];
//    NSMutableArray *addresses = [NSMutableArray array];
//    while ([set next]) {
//        OrderAddress *address = [[OrderAddress alloc] init];
//        //address.ID = [set intForColumn:@"id"];
//        address.name = [set stringForColumn:@"name"];
//        address.phone_number = [set stringForColumn:@"phone_number"];
//        address.zipCode = [set stringForColumn:@"zipCode"];
//        address.country_address = [set stringForColumn:@"country_address"];
//        address.detail_address = [set stringForColumn:@"detail_address"];
//        address.is_default = [set intForColumn:@"is_default"];
...
//}
//
//+ (OrderAddress *)addressIs_default
//{
//    NSString *sql =[NSString stringWithFormat:@"SELECT * FROM t_address WHERE is_default = 1"] ;
//    FMResultSet *set = [_db executeQuery:sql];
//    NSMutableArray *addresses = [NSMutableArray array];
//    while ([set next]) {
//        OrderAddress *address = [[OrderAddress alloc] init];
...
//        address.country_address = [set stringForColumn:@"country_address"];
//        address.detail_address = [set stringForColumn:@"detail_address"];
//        address.is_default = [set boolForColumn:@"is_default"];
//        [addresses addObject:address];
//    }
...
//}
//
//+ (BOOL)updateAddress:(OrderAddress *)address
//{
//    //NSNumber *ID = @(address.ID);
//    NSString *name = address.name;
//    NSString *phone_number = address.phone_number;
//    NSString *zipCode = address.zipCode;
//    NSString *country_address = address.country_address;
//    NSString *detail_address = address.detail_address;
//    NSNumber *is_default = @(address.is_default);
//    
//    BOOL flag = [_db executeUpdate:@"UPDATE t_address SET name = ?, phone_number = ?, zipCode = ?, country_address = ?, detail_address = ?,is_default = ? WHERE id = ?", name, phone_number, zipCode, country_address, detail_address, is_default, 0];
//    
//    return flag;
//
//}
//+ (BOOL)addNewAddress:(OrderAddress *)address
//{
//    NSString *name = address.name;
//    NSString *phone_number = address.phone_number;
//    NSString *zipCode = address.zipCode;
...
//    NSNumber *is_default = @(address.is_default);
//    
//    BOOL flag = [_db executeUpdate:@"INSERT INTO t_address (name, phone_number, zipCode, country_address, detail_address ,is_default) VALUES (?, ?, ?, ?, ?, ?)", name, phone_number, zipCode, country_address, detail_address, is_default];
//    
//    return flag;
//
//}
