...
- (HooMoment *)moment
{
    if (_moment == nil) {
        ...
        NSString *photoName = [NSString stringWithFormat:@"MomentPhoto_%ld.jpg",currentPhotoID];
        NSString *photo_thumb_Name = [NSString stringWithFormat:@"MomentPhoto_%ld_thumb.jpg",currentPhotoID];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (section == 1 && self.showHeaderView) {
        ...
        headerView.frame = CGRectMake(0, 0, self.tableView.bounds.size.width, self.tableView.bounds.size.height);
...
- (void)autoCreateMomentAndShowHeaderView
{
    self.momentImage = [UIImage getRandomImageFromPhotoLibrary:^(NSString *failAuthorizationInfo) {
        [self showNoAccessAlertAndCancel:failAuthorizationInfo];
    }];
